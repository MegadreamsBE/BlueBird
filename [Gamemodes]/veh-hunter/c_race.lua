...
addEvent("setCurrentMatrix",true)
addEventHandler("setCurrentMatrix",g_Root,
	function()
		local x,y,z,camX,camY,camZ = getCameraMatrix()
		setCameraMatrix(x,y,z,camX,camY,camZ)
		setElementDimension(localPlayer,getElementData(localPlayer,"bb.room"))
	end
)
...
function renderTimeLeft()
	local roomState = getElementData(getElementByID("room_"..tostring(getElementData(localPlayer,"bb.room"))),"room.state")
		if(roomState ~= "playing") then
			return
		end
	dxDrawImage(((screenX/2)-(conv(150)/2)), 0, conv(150), conv(50), 'images/timeleft.png', 0, 0, 0)
	local distLeft = endTime-(getTickCount())
	
	if(distLeft < 0) then
		distLeft = 0
		local length = dxGetTextWidth("Time is up!",conv(2),"default-bold")
		local height = dxGetFontHeight(conv(2),"default-bold")
	
		dxDrawText("Time is up!", ((screenX/2)-(length/2)), ((screenY-conv((230))+conv(75))-(height/2)), (((screenX/2)-(length/2))+50), screenY-conv(150), tocolor(255,0,0,255), conv(2), "default-bold" )
		local x,y,z,camX,camY,camZ = getCameraMatrix()
		setCameraMatrix(x,y,z,camX,camY,camZ)
	else
		if(distLeft < 10000) then
			local length = dxGetTextWidth("Hurry up!!!",conv(2),"default-bold")
			local height = dxGetFontHeight(conv(2),"default-bold")
		
			dxDrawText("Hurry up!!!", ((screenX/2)-(length/2)), ((screenY-conv((230))+conv(75))-(height/2)), (((screenX/2)-(length/2))+50), screenY-conv(150), tocolor(255,0,0,255), conv(2), "default-bold" )
		end
	end
	
	local timeLeft = formatTime(distLeft)
	local timePlayed = 0
	
	if(lastTick ~= 0) then
		if(getElementData(localPlayer,"state") == "alive" and distLeft > 0) then
			lastTick = getTickCount()
		end
		
		timePlayed = formatTime((lastTick-startTime))
	else
		timePlayed = formatTime(0)
	end
	
	local width = dxGetTextWidth(timeLeft,conv(0.9),"default-bold")
	dxDrawText(timeLeft, ((screenX/2)-conv(35)-conv(width/2)), conv(27), 0, 0, tocolor(255,255,255,200), conv(0.9), "default-bold","left","top",false,false,false,true)
	
	local width = dxGetTextWidth(timePlayed,conv(0.9),"default-bold")
	dxDrawText(timePlayed, ((screenX/2)+conv(35)-conv(width/2)), conv(27), 0, 0, tocolor(255,255,255,200), conv(0.9), "default-bold","left","top",false,false,false,true)
end
addEventHandler("onClientRender",g_Root,renderTimeLeft)
...
function renderDeathlist()
	...
	local roomState = getElementData(getElementByID("room_"..tostring(getElementData(localPlayer,"bb.room"))),"room.state")
	if(roomState == "loading" or roomState == "unloading") then
		return
	end
	...
	for _,v in ipairs(g_DeathListRec) do
		...
		if(camTarget == false) then
			r,g,b = 0,0,0
			alpha = 100
		elseif(getVehicleOccupant(camTarget) == false) then
			r,g,b = 0,0,0
			alpha = 100
		elseif(getVehicleOccupant(camTarget) == getElementData(v.player,"spectating")) then
			r,g,b = 27,161,226
			alpha = 40
		else
			r,g,b = 0,0,0
		...
		if(v.player == localPlayer) then
			...
			dxDrawRectangle((screenX-v.recX), ((screenY/2)-recPos), conv(200), conv(30), tocolor(r,g,b,alpha))
		end
...
function moveWonCamera()
	...
	if(getElementData(getElementByID("room_"..tostring(getElementData(localPlayer,"bb.room"))),"room.state") ~= "winning") then
		return
	end
...
addEventHandler("onMapIsLoaded",g_Root,
	function ()
		...
		elseif(pID <= (#g_Spawnpoints*2)) then
			...
			local pPos = math.random(1,#g_Spawnpoints)
	...
	function (aliveTable,ghostmode)
		...
		for i,plr in ipairs(aliveTable) do
			for i,veh in ipairs(getElementsByType("vehicle")) do
				setElementCollidableWith(getPedOccupiedVehicle(plr), veh, state)
				setElementCollidableWith(veh, getPedOccupiedVehicle(plr), state)
			end
		end
	end
)
...
function doSetTimeLeft(dif)
	endTime = getTickCount() + dif
end
addEvent("setTimeLeft",true)
addEventHandler("setTimeLeft",g_Root,doSetTimeLeft)

function renderCount()
	if(getElementData(getElementByID("room_"..tostring(getElementData(localPlayer,"bb.room"))),"room.state") ~= "countdown") then
		return
	end
		...
		if(player == localPlayer) then
			for _,v in ipairs(g_Pickups) do
				if(v.colshape == source) then
					g_PrevVehicleHeight = getElementDistanceFromCentreOfMassToBaseOfModel(getPedOccupiedVehicle(localPlayer))
