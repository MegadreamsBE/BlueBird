...
addEvent("onRequestServiceDownload",true)
addEventHandler("onRequestServiceDownload", g_ResourceRoot,
	function(player,filename)
		local file = fileOpen(filename)
		local buffer = ""
		while not fileIsEOF(file) do
		...
		triggerLatentClientEvent(player,"onClientDownloadedService", 1*10^8,g_ResourceRoot, buffer,filename) -- 0.1MB/s
	end
)